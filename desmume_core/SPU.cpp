...
//static ISynchronizingAudioBuffer* NULL = metaspu_construct(ESynchMethod_Z);
//static ISynchronizingAudioBuffer* NULL = metaspu_construct(ESynchMethod_N);
...
//		NULL = metaspu_construct(synchmethod);
	}
...
void SPU_Emulate_core()
{
	...
	{
		soundProcessor->FetchSamples(SPU_core->outbuf, spu_core_samples, synchmode, NULL);
	}
	else
	{
		SPU_DefaultFetchSamples(SPU_core->outbuf, spu_core_samples, synchmode, NULL);
	}
}
...
void SPU_Emulate_user(bool mix)
{
	...
	{
		processedSampleCount = soundProcessor->PostProcessSamples(postProcessBuffer, freeSampleCount, synchmode, NULL);
	}
	else
	{
		processedSampleCount = SPU_DefaultPostProcessSamples(postProcessBuffer, freeSampleCount, synchmode, NULL);
	}
...
void SPU_DefaultFetchSamples(s16 *sampleBuffer, size_t sampleCount, ESynchMode synchMode, ISynchronizingAudioBuffer *theNULL)
{
	if (synchMode == ESynchMode_Synchronous)
	{
		theNULL->enqueue_samples(sampleBuffer, sampleCount);
	}
}

size_t SPU_DefaultPostProcessSamples(s16 *postProcessBuffer, size_t requestedSampleCount, ESynchMode synchMode, ISynchronizingAudioBuffer *theNULL)
{
	size_t processedSampleCount = 0;
	...
	switch (synchMode)
	{
		...
		case ESynchMode_Synchronous:
			processedSampleCount = theNULL->output_samples(postProcessBuffer, requestedSampleCount);
			break;
...
void SNDDummyFetchSamples(s16 *sampleBuffer, size_t sampleCount, ESynchMode synchMode, ISynchronizingAudioBuffer *theNULL);
size_t SNDDummyPostProcessSamples(s16 *postProcessBuffer, size_t requestedSampleCount, ESynchMode synchMode, ISynchronizingAudioBuffer *theNULL);
...
void SNDDummyClearBuffer() {}
void SNDDummyFetchSamples(s16 *sampleBuffer, size_t sampleCount, ESynchMode synchMode, ISynchronizingAudioBuffer *theNULL) {}
size_t SNDDummyPostProcessSamples(s16 *postProcessBuffer, size_t requestedSampleCount, ESynchMode synchMode, ISynchronizingAudioBuffer *theNULL) { return 0; }
