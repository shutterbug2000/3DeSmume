...
inline void FlipByteOrder(u8 *src, unsigned int count)
{
	u8 *start=src;
	u8 *end=src+count-1;
...
inline unsigned int float_to_u32(float f) {
	union {
		unsigned int a;
		float b;
	} fuxor;
...
inline float u32_to_float(unsigned int u) {
	union {
		unsigned int a;
		float b;
	} fuxor;
...
inline void en32lsb(u8 *buf, unsigned int morp)
{ 
	buf[0]=(u8)(morp);
	buf[1]=(u8)(morp>>8);
	buf[2]=(u8)(morp>>16);
...
inline unsigned int de32lsb(u8 *morp)
{
...
FORCEINLINE s64 fx32_mul(const signed int a, const signed int b)
{
#ifdef _WIN32
	return __emul(a,b);
#else
...
FORCEINLINE signed int fx32_shiftdown(const s64 a)
{
#ifdef _WIN32
	return (signed int)__ll_rshift(a,12);
#else
	return (signed int)(a>>12);
#endif
}

FORCEINLINE s64 fx32_shiftup(const signed int a)
{
#ifdef _WIN32
