...
u8 GPU_tempScreen[4*256*192];
u8 GPU_tempScanline[4*256]; //only one scanline
int GPU_tempScanline_valid; //whether GPU_tempScanline is valid (if not, fetch it from GPU_screen)
...
int Screen_Init(int coreid) {
   MainScreen.gpu = GPU_Init2(0);
   SubScreen.gpu = GPU_Init2(1);

   return GPU_ChangeGraphicsCore(coreid);
}

void Screen_Reset(void) {
   GPU_Reset2(MainScreen.gpu, 0);
   GPU_Reset2(SubScreen.gpu, 1);
}
					...
					if (gpu->core == GPU_MAIN)
					{
						if (i16 == 0 && dispCnt->BG0_3D)
						{
							...
							gpu3D->NDS_3D_GetLine (l, start, end, (u16*)dst+ofs);
                           	//modeRender[dispCnt->BG_Mode][i16](gpu, i16, l, (u16*)dst+ofs);
							continue;
						}
					}
